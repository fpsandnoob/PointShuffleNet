...
def cuda_batch_fps_sampling(xyz, npoint):
    """
        Input:
            xyz: pointcloud data, [B, N, 3]
            npoint: number of samples
    ...
    batch_idx = torch.arange(0, B, device=flatten_xyz.device)
...
def sample_and_group(npoint, radius, nsample, xyz, points, returnfps=False, use_cluster=False):
    """
    Input:
        npoint:
        radius:
    ...
    if not use_cluster:
        ...
        # torch.cuda.empty_cache()
        # new_xyz = cuda_batch_fps_sampling(xyz, npoint)
    else:
        clusterfps = ClusterFPS(npoint=npoint, iteration=10, num_centers=16, fixed=True, device=torch.device("cuda"))
        new_xyz, centers, _, _, _ = clusterfps.forward(xyz)
    ...
    if returnfps:
        ...
        return new_xyz, new_points, centers
    else:
...
class PointNetSetAbstraction(nn.Module):
    def __init__(self, npoint, radius, nsample, in_channel, mlp, group_all, use_cluster=False, shuffle=False):
    ...
    def forward(self, xyz, points):
        ...
        for i, conv in enumerate(self.mlp_convs):
            ...
            if conv.in_channels == conv.out_channels:
                new_points = F.relu(bn(conv(new_points))) + new_points
                if self._shuffle:
                    new_points = self.shuffle(new_points)
            else:
