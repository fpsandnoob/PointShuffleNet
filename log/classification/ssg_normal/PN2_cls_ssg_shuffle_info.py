...
class get_loss(nn.Module):
    ...
    def forward(self, pred, target, model: torch.nn.Module, **kwargs):
        info_losses = []
...
if __name__ == '__main__':
    model = get_model(40, normal_channel=True).cuda()
    input = torch.randn(24, 6, 1024).cuda()
    # macs, params, times = profile(model, inputs=(input,), iter=1)
    # print(clever_format([macs, params], "%.3f"), times)
    summary(model, input)
    # model(input)
